...
export default function EditLead() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push('/');
    } else if (isAuthenticated && leadId) {
      fetchLead();
    }
  }, [isAuthenticated, isLoading, router, leadId]);
  ...
  const fetchLead = async () => {
    try {
      ...
      setFormData({
        businessOwnerName: response.lead.businessOwnerName || '',
        website: response.lead.website || '',
        address: response.lead.address || '',
        phoneNumber: response.lead.phoneNumber || '',
        problem: response.lead.problem || '',
        niche: response.lead.niche || 'coaching',
        status: response.lead.status || 'not-contacted',
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        <Card>
          <CardHeader className="px-4 sm:px-6">
            <CardTitle className="text-xl sm:text-2xl">Edit Lead</CardTitle>
            <CardDescription className="text-sm">Update the lead information</CardDescription>
          </CardHeader>
